package fr.ac_versailles.crdp.apiscol.previews.filters;
...
import fr.ac_versailles.crdp.apiscol.previews.PreviewsApi;
...
public class SecurityFilter implements ContainerRequestFilter {
	...
	public ContainerRequest filter(ContainerRequest request) {
		...
		if (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.POST)
			...
			String secret = PreviewsApi.getProperty(ParametersKeys.sharedSecret, context);
			...
			if (!StringUtils.equals(providedHashedEtag, computedHashedEtag))
				throw new WebApplicationException(Response.status(401)
						.entity("Invalid authorization token in previews web service ").build());
		}
		return request;
	}
}
